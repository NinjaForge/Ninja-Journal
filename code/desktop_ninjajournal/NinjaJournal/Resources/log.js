var Teamlog = new Class({
	...
	attachEvents: function() {
		...
		trg.addEvent('click', function(e) {
			...
			this.todo.options.transition = Fx.Transitions.Cubic[this.todo.open ? 'easeIn' : 'easeOut'];
	...
	getTodos: function() {
		...
		new Ajax(this.url+'&layout=todos', {
			...
			onComplete: function(){
				...
				var length = $('todos-form').getElement('ul').getChildren().length, text = $('todos-trigger').get('text');
				$('todos-trigger').set('text', text.replace(/[0-9]+/g, length));
			}
		}).request();
	},
